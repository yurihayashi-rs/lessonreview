...
import { START_FETCH, FETCH_SUCCESS, ERROR_FOUND, INPUT_EDIT, TOGGLE_MODE, } from "./actionLists";
...
const loginReducer = (state, action) => {
    switch (action.type) {
        ...
        case ERROR_FOUND: {
            return {
            ...state,
            error: "メールアドレスまたはパスワードが正しくありません。パスワードは5文字以上必要です",
            isLoading: false,
            };
        }
        case INPUT_EDIT: {
    ...
    const login = async (event) => {
        ...
        if (state.isLoginView) {
            try {
                ...
                res.data.access
                ? (window.location.href ="/lessonvideos")
